...
try {
    ...
    try {
        if (typeof window !== 'undefined') {
            // Pro lokální vývoj použijte debug token
        ...
        const appCheck = initializeAppCheck(app, {
            provider: new ReCaptchaV3Provider(siteKey || 'dummy-site-key'),
            isTokenAutoRefreshEnabled: true,
        });
        window.firebaseAppCheck = appCheck;
        console.log('✅ Firebase App Check inicializován');
        if (!siteKey) {
            console.warn('⚠️ App Check běží s debug tokenem nebo dummy site key. Pro produkci nastavte window.FIREBASE_RECAPTCHA_V3_SITE_KEY.');
        }
    } catch (err) {
